...
- **Metadata-only mutations:** Group creation and label, tag, grouping, recurrence, completion, or sort-order changes do not propagate because they cannot change the financial chain. Entry and recurring-expense edits compare the submitted amount with the stored numeric value so forms may safely resend an unchanged amount without triggering carry-over.
...
- **Atomicity:** The initiating mutation and the full downstream chain run inside `withFinancialTransaction`, using one transaction client for ownership checks, snapshots, and writes. A user-row lock serializes financial requests before dependent reads, including year creation/deletion. Failures roll back the originating mutation and every downstream balance/version change. See [Atomic Financial Writes](financial-transactions.md).
...
- **Missing data:** When propagation requires a source or intermediate snapshot, missing data throws and rolls back the operation instead of silently reporting success. Missing December also rejects the operation.
...
- **Creation repair:** When a later year is created, propagation starts from its latest predecessor after insertion, using the same transaction and user lock as predecessor reads and year creation.
