...
- Additional income, expense, and expense-group rows can be completed once the expected transaction occurs. Incomplete entries and groups retain their contextual open lock. Completed ungrouped and grouped entries replace Delete with a compact closed lock in the resting row, while completed groups replace header Delete with the authoritative closed lock so they can be reopened while collapsed. Optimistic pending remains visible and disabled, and the group header keeps a stable height on mobile and desktop. A completed group still effectively locks its child entries without overwriting their individual completion state.
