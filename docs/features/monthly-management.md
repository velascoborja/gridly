...
- Additional income, expense, and expense-group rows can be completed once the expected transaction occurs. Completing an entry closes its editor; completing a group collapses its body. Failed completion restores the prior open view so the existing error remains visible, while reopening does not auto-expand. Completed entries replace Delete with a compact closed lock in the resting row, and completed groups use the authoritative header lock. During the optimistic request, a centered spinner replaces the lock icon without changing the action footprint. A successful response then articulates the lock shackle for 300ms; reopening keeps the temporary open lock in place until that confirmation finishes before restoring the normal row or header action. Reduced-motion users receive the final state immediately. Group headers keep a uniform 48px height across breakpoints, and a completed group still locks its child entries without overwriting their individual completion state.
