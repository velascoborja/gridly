...
- Additional income and expense cards remain fully expanded and editable across all months while exposing a three-state temporal treatment. Past months use a cooler slate low-elevation surface and leading edge, the current month uses the standard card surface, and future months use an indigo forecast surface and leading edge. Past and forecast cards include their localized status beside the title.
...
- After most successful monthly mutations, `MonthOverview` recomputes the full month chain and lifts the updated `YearData` to `YearPageClient`, preserving the mounted client state while navigating between months without calling `router.refresh()`. Group creation and recurring expense tag changes retain their own refresh calls. Local workspace updates do not guarantee fresh data after leaving and restoring a route: `staleTimes.dynamic = 0` does not disable back/forward caching. See [Language Changes and Route Refreshes](internationalization.md#language-changes-and-route-refreshes) for the retained calls and their synchronization requirements.
...
- `YearPageClient` loads `MonthOverview`, `AnnualView`, and `SettingsForm` as separate client chunks. A direct month visit loads the monthly workspace without the annual charts or settings code; the first switch to another view shows an in-shell skeleton while its chunk loads. Switching between months never crosses this lazy boundary because all 12 months reuse the already-loaded `MonthOverview` and in-memory `YearData`.
- Direct month-route loading and the deferred `MonthOverview` chunk reuse the same monthly skeleton. It mirrors the resting workspace with the month strip, KPI header, and two additional-entry cards; fixed-value editors stay absent because the loaded editable view starts with that panel collapsed. The direct route also reserves the final app-header controls to avoid layout shifts during hydration.
