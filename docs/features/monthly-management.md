...
- Additional income, expense, and expense-group rows can be completed once the expected transaction occurs. Completion uses an animated lock shown only after opening an entry or expanding a group, keeps the row in its current order, and blocks every mutation except reopening. Entry locks share the editor action row on desktop and mobile. Group locks share the compact action bar on mobile and sit at the bottom-right opposite the bottom-left "Add entry" action on desktop. A completed group also effectively locks its child entries without overwriting their individual completion state; group deletion remains visible in the incomplete group header on mobile and desktop.
