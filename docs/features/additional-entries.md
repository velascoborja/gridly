...
- **Moving Entries:** In editable monthly views, ungrouped additional income and expense rows can be moved to another month in the same year from the entry edit row's "Move to month" menu. Desktop users can also drag an entry onto another month in the month strip. Ungrouped expenses can additionally be dragged onto an eligible expense group in the current month; the existing "Move to group" selector remains available for touch and keyboard use. A grouped entry cannot move independently while retaining its old group: the API requires the request to remove it from the group or assign a group in the destination month. Moving changes ownership only; it does not copy or duplicate the entry.
...
The collection and item APIs enforce the same model as the UI: grouped entries must have type `expense`, must share the group's `monthId`, and cannot be recurring. Assigning an ungrouped recurring expense to a group clears recurrence. Requests that explicitly combine `groupId` with `type: "income"` or `isRecurring: true` return HTTP `400`.
