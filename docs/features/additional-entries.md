...
- **Completion lock:** Incomplete income and expense rows expose the animated open lock as a typed `command` in `EntryFormRow.actions`. Completing an entry optimistically closes its editor and returns it to the resting row, where Delete is replaced by a compact closed lock for direct reopen; this applies to ungrouped and grouped entries. Pending locks retain their action position, stay busy and disabled, and replace the lock icon with a centered spinner. After a successful `PATCH`, the shackle animates closed or open for 300ms; a reopened row keeps the temporary open lock until that confirmation ends, then restores its normal action without expanding the editor. Reduced-motion users skip the articulation and transient wait. If completion fails, no confirmation animation runs: the prior completion state and open editor are restored so the localized error is visible. Completed rows remain visually muted and cannot be edited, tagged, moved, regrouped, deleted, or dragged until reopened. The API enforces the same rule and returns `409 { error: "completed_locked" }` for blocked mutations.
