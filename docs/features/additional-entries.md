...
- **Completion lock:** Incomplete income and expense rows expose the animated open lock inside their contextual editor through `EntryFormRow.completionAction`. Completing an entry optimistically closes its editor and returns it to the resting row, where Delete is replaced by a compact closed lock for direct reopen; this applies to ungrouped and grouped entries. If completion fails, the prior completion state and open editor are restored so the localized error is visible. Reopening does not automatically expand the editor. Pending locks retain the trailing action slot, stay busy and disabled, and reconcile or roll back through the existing entry `PATCH` endpoint. Completed rows remain visually muted and cannot be edited, tagged, moved, regrouped, deleted, or dragged until reopened. The API enforces the same rule and returns `409 { error: "completed_locked" }` for blocked mutations.
