...
- **Completion lock:** Incomplete income and expense rows expose the animated open lock inside their contextual editor through `EntryFormRow.completionAction`. After completion hides Delete, the resting row replaces Delete with a compact closed lock so the resource can be reopened without opening its editor. The same replacement applies to entries inside editable expense groups. During optimistic reopen, the lock retains the trailing action slot, becomes busy and disabled, and reconciles or rolls back through the existing entry `PATCH` endpoint. Completed rows remain visually muted and cannot be edited, tagged, moved, regrouped, deleted, or dragged until reopened; their editor remains available as a secondary reopen path with disabled financial fields, hidden Save and mutation actions, and Exit in place of Cancel. The API enforces the same rule and returns `409 { error: "completed_locked" }` for blocked mutations.
...
- Completing a group replaces its header Delete action with a compact closed lock, making reopen available while the group is collapsed. The header preserves the same mobile and desktop height across incomplete, pending, and completed states. Expanded open-lock controls are shown only for incomplete groups, so the completed group has a single authoritative reopen action in its header.
