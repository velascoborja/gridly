...
async function loadBuildWorkbook() {
  ...
  await transpileModuleToTemp("src/lib/tag-stats.ts", outDir);
...
const yearData = {
  ...
  months: Array.from({ length: 12 }, (_, index) => {
    const month = index + 1;
    const additionalExpense = month === 1 ? [{ id: 1, monthId: 1, type: "expense", label: "Seguro", amount: 25, tagId: null, tag: null }] : [];
...
test("excel export produces a 13-sheet workbook with summary first and one sheet per month", async () => {
  ...
  try {
    ...
    // Should have 1 summary sheet + 1 categories sheet (year has additional expenses) + 12 month sheets
    ...
    // Month sheets exist with correct names (categories sheet is at index 1)
    assert.equal(workbook.worksheets[1].name, "Categorías");
    assert.equal(workbook.worksheets[2].name, "Enero");
    assert.equal(workbook.worksheets[13].name, "Diciembre");

    // December sheet has fixed and additional income subsections
    const decemberSheet = workbook.worksheets[13];
