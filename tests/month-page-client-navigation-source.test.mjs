...
test("authenticated year routes use a client shell for in-year navigation", async () => {
  ...
  assert.match(shellSource, /window\.history\.pushState/, "year client shell should update local view URLs without server navigation");
  assert.match(shellSource, /usePathname/, "year client shell should derive locale-aware path prefixes from the current route");
  assert.doesNotMatch(shellSource, /useRouter/, "year client shell should not force App Router navigations for local view changes");
