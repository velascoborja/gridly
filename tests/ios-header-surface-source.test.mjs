...
test("app header exposes a matching iOS status bar surface", async () => {
  ...
  assert.match(layoutSource, /themeColor:\s*"#e8edf3"/, "Safari should use a header-matched theme color");
  ...
  assert.match(globalsSource, /\.app-header-surface::before\s*\{/, "safe area overlay should be defined");
  assert.match(globalsSource, /height:\s*env\(safe-area-inset-top\)/, "safe area overlay should cover the iOS status bar area");
});
