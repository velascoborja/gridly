...
function writeMonthSection(ws, year, month, startRow) {
    const sheetName = utils_1.MONTH_NAMES[month.month - 1];
    ...
    if (month.additionalPayslip > 0)
        addIncRow("Paga extra", month.additionalPayslip);
    if (month.bonus > 0)
        addIncRow("Bonus", month.bonus);
...
async function buildWorkbook(yearData) {
    ...
    yearData.months.forEach((month, index) => {
        ...
        ws.getCell(`A${row}`).value = utils_1.MONTH_NAMES[month.month - 1];
